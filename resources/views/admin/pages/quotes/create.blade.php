...
@section('content')
    ...
    <div class="dossier-card">
        <h3 class="dossier-card-title">
            <i class="bi bi-file-earmark-plus text-primary"></i> Create New Quote
        </h3>
        ...
        <form action="{{ route('admin.quotes.store') }}" method="POST">
            ...
            <div class="row g-3">
                <div class="col-md-6">
                    ...
                    <select class="form-select" id="lead_id" name="lead_id" required>
                        ...
                        @foreach($leads as $leadOption)
                            <option value="{{ $leadOption->id }}" {{ (old('lead_id', $selectedLead?->id) == $leadOption->id) ? 'selected' : '' }}>
                ...
                <div class="col-md-6">
                    ...
                    <select class="form-select" id="currency_id" name="currency_id" required>
                        ...
                        @foreach($currencies as $currencyOption)
                            <option value="{{ $currencyOption->id }}" {{ (old('currency_id', $selectedLead?->currency_id) == $currencyOption->id) ? 'selected' : '' }}>
                ...
                <div class="col-md-6">
                    ...
                    <input type="number" step="0.01" class="form-control" id="budget_min" name="budget_min" value="{{ old('budget_min') }}">
                </div>

                <div class="col-md-6">
                    <label for="budget_max" class="form-label fw-semibold">Maximum Budget</label>
                    <input type="number" step="0.01" class="form-control" id="budget_max" name="budget_max" value="{{ old('budget_max') }}">
                </div>
            ...
            <div class="mt-4 text-end">
                <a href="{{ route('admin.quotes') }}" class="btn btn-light">Cancel</a>
