                        ...
                        <tr>
                                    ...
                                    <div class="lead-name-box">
                                        ...
                                        <div class="lead-contact-meta">
                                            <span class="lead-contact-item text-muted"
                                                title="Prepared by {{ $quote->creator?->name ?? 'System' }}">
                                                <i class="bi bi-person"></i>
                                                <span
                                                    class="lead-contact-text">{{ $quote->creator?->name ?? 'System' }}</span>
                                            </span>
                                            <span class="lead-contact-item text-muted"
                                                title="Created at {{ $quote->created_at->format('M d, Y') }}">
                                                <i class="bi bi-calendar3"></i>
                                                <span
                                                    class="lead-contact-text">{{ $quote->created_at->format('M d, Y') }}</span>
                                            </span>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <td>
                                @if ($quote->lead)
                                    ...
                                    @if ($quote->lead->email)
                                        <div class="lead-contact-meta">
                                            <a href="mailto:{{ $quote->lead->email }}" class="lead-contact-item"
                                                title="{{ $quote->lead->email }}">
    ...
    @else
        ...
        <td>
            ...
            @if ($quote->budget_min && $quote->budget_max)
                <span class="lead-budget-text">{{ $itemSym }}{{ number_format($quote->budget_min, 0) }} –
                    {{ $itemSym }}{{ number_format($quote->budget_max, 0) }}</span>
        ...
        <td>
            @can('edit-quotes')
                @if (in_array($quote->status, [
                        \App\Enums\QuoteStatus::Accepted,
                        \App\Enums\QuoteStatus::Rejected,
                        \App\Enums\QuoteStatus::Expired,
                    ]))
                    <span class="status-badge {{ $quote->status->badgeClass() }}" title="Status is finalized and locked">
                ...
                @else
                    <form action="{{ route('admin.quotes.update-status', $quote) }}" method="POST" class="d-inline">
                        ...
                        <select name="status" onchange="this.form.submit()"
                            class="lead-status-select {{ $quote->status->badgeClass() }}">
                            @foreach ($quote->status->allowedTransitions() as $allowedOption)
                                <option value="{{ $allowedOption->value }}"
                                    {{ $quote->status->value === $allowedOption->value ? 'selected' : '' }}>
                                    {{ $allowedOption->label() }}
                                </option>
                            @endforeach
                        </select>
        ...
        <td class="text-end-align nowrap-cell">
            <div class="table-actions">
                @can('view-quotes')
                    <a href="{{ route('admin.quotes.show', $quote) }}" class="btn-action btn-action-view btn-action-icon"
                ...
                @can('edit-quotes')
                    <a href="{{ route('admin.quotes.edit', $quote) }}" class="btn-action btn-action-edit btn-action-icon"
                        title="Edit Quote">
                ...
                @can('delete-quotes')
                    <form action="{{ route('admin.quotes.destroy', $quote) }}" method="POST" class="d-inline"
                        ...
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="btn-action btn-action-delete btn-action-icon" title="Delete Quote">
