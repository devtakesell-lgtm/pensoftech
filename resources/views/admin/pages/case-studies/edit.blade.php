    ...
    <form action="{{ route('admin.case-studies.update', $caseStudy) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="row">
            <div class="col-lg-8">
                <div class="card mb-4">
                    <div class="card-body">
                            ...
                            <select name="project_id" id="project_id" class="form-select" required>
                                ...
                                @foreach ($projects as $project)
                                    <option value="{{ $project->id }}"
                                        {{ old('project_id', $caseStudy->project_id) == $project->id ? 'selected' : '' }}>
                        ...
                        @php
                            $existingChallenges = $caseStudy->challenges->map(function ($challenge) {
                                return [
                                    'title' => $challenge->title,
                                    'description' => $challenge->description,
                                    'solutions' => $challenge->solutions->map(function ($solution) {
                                        return [
                                            'description' => $solution->description,
                                            'status' => $solution->status?->value ?? '',
                                        ];
                                    })->toArray() ?: [['description' => '', 'status' => '']],
                                ];
                            })->toArray();

                            if (empty($existingChallenges)) {
                                $existingChallenges = [['title' => '', 'description' => '', 'solutions' => [['description' => '', 'status' => '']]]];
                            }
                        @endphp
                        <div class="mb-4" x-data="{
                            challenges: {{ json_encode(old('challenges', $existingChallenges)) }},
                            addChallenge() {
                        ...
                        <div class="mb-4">
                            <label for="result" class="form-label fw-semibold">The Result <span
                                    class="text-danger">*</span></label>
                            <textarea name="result" id="result" class="form-control" rows="3" required>{{ old('result', $caseStudy->result) }}</textarea>
                        </div>

                        <div class="mb-4">
                            <label for="content" class="form-label fw-semibold">Detailed Content (Story)</label>
                            <textarea name="content" id="content" class="form-control">{{ old('content', $caseStudy->content) }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="col-lg-4">
                ...
                @php
                    $existingMetrics = $caseStudy->metrics
                        ->map(function ($m) {
                            return [
                    ...
                    if (empty($existingMetrics)) {
                        $existingMetrics = [['metric_name' => '', 'metric_value' => '', 'metric_suffix' => '']];
                    }
                @endphp

                <div class="card mb-4" x-data="{
                    metrics: {{ json_encode(old('metrics', $existingMetrics)) }},
                    addMetric() {
                ...
                <div class="card mb-4">
                    <div class="card-body">
                        ...
                        <x-admin.components.forms.image-upload name="image" label="Cover Image" aspect="banner"
                            :current="$caseStudy->featured_image ? Storage::url($caseStudy->featured_image) : null"
                            help="Leave blank to keep current image. Recommended: 1200x600px. JPG, PNG, WEBP." />
                    </div>
                </div>
                    ...
                    <div class="card-body">
                            ...
                            <select name="status" id="status" class="form-select" required>
                                @foreach (\App\Enums\ContentStatus::cases() as $status)
                                    <option value="{{ $status->value }}"
                                        {{ old('status', $caseStudy->status->value) === $status->value ? 'selected' : '' }}>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="date" name="published_at" id="published_at" class="form-control"
                                value="{{ old('published_at', $caseStudy->published_at?->format('Y-m-d')) }}">
                        </div>
