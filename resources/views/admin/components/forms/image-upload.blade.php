...
@php
    ...
    // Map abstract aspect names to inline CSS aspect-ratio since Bootstrap doesn't have native utility for all
    $aspectStyle = match ($aspect) {
        'wide' => 'aspect-ratio: 16/7; max-height: 250px;',
        'banner' => 'aspect-ratio: 21/9; max-height: 200px;',
        'video' => 'aspect-ratio: 16/9; max-height: 250px;',
        default => 'aspect-ratio: 1/1; max-height: 250px;',
    };
@endphp
...
<div class="mb-3">
    ...
    <div
        ...
        :class="isDragging ? 'border-primary bg-light' : '{{ $hasError ? 'border-danger bg-danger-subtle' : 'border-secondary bg-white' }}'"
        class="position-relative overflow-hidden rounded-3 border border-2 border-dashed transition-all"
        style="transition: all 0.2s ease;"
    >
        <div class="position-relative w-100 overflow-hidden d-flex align-items-center justify-content-center" style="{{ $aspectStyle }} background-color: #f8f9fa;">
            ...
            >
                <div class="d-flex align-items-center justify-content-center rounded-circle bg-light text-primary mb-2" style="width: 50px; height: 50px;">
            ...
            >
                <span class="badge bg-light text-dark shadow-sm px-3 py-2 rounded-pill border">
                    <i class="bi bi-camera me-1"></i> <span x-text="fileName ? 'Change selected image' : 'Change image'"></span>
                </span>
            </div>
        </div>
    ...
    <div x-show="fileName" x-cloak class="mt-2 d-flex align-items-center gap-2 rounded border border-success bg-success-subtle px-3 py-2 text-success" style="font-size: 0.85rem;">
